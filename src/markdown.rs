...
pub fn render<CTX: 'static, M>(source: &str) -> Html<CTX, M> where M: Component<CTX> {
    let mut stack: Vec<VTag<CTX, M>> = Vec::new();
    let mut active_tag: VTag<CTX, M> = VTag::new("markdown");
    for ev in Parser::new_ext(source, OPTION_ENABLE_TABLES) {
        match ev {
            ...
            Event::Text(text) => {
                let node = VText::new(String::from(text)).into();
                active_tag.add_child(node);
            }
            Event::SoftBreak => {
                let node = VText::new(String::from("\n")).into();
                active_tag.add_child(node);
            },
            Event::HardBreak => {
...
fn make_tag<T, M>(t: Tag) -> VTag<T, M> where M: Component<T> {
    match t {
